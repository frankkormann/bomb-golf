...
void Border_Draw(float x, float y, float depth, float width, float height) {
	for (float cx = x; cx < x + width; cx += BORDER_TILE_SIZE) {
		SpriteSheet_Draw(SPRITE_GUI_BORDER, cx, y - BORDER_TILE_SIZE, depth,
				M_PI/2, true, false);
		SpriteSheet_Draw(SPRITE_GUI_BORDER, cx, y + height, depth,
				M_PI/2, false, false);
	}
	for (float cy = y; cy < y + height; cy += BORDER_TILE_SIZE) {
		SpriteSheet_Draw(SPRITE_GUI_BORDER, x - BORDER_TILE_SIZE, cy, depth,
				0, true, false);
		SpriteSheet_Draw(SPRITE_GUI_BORDER, x + width, cy, depth,
				0, false, false);
	}
